...
if __name__=='__main__':
    ...
    daily_data_url = 'https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_month.geojson'
    bucket_name="earthquake_analysissd"
    destination_blob_name=f'Dataflow/landing/{current_date}/Historical_data.json'
    ...
    with beam.Pipeline(options=options) as p1:
        res1=(p1 | "Read data from GCS" >> beam.io.ReadFromText(gcs_path)
                 ...
                 | "Write data to GCS as Parquet" >>beam.io.WriteToParquet(file_path_prefix=f'gs://{bucket_name}/Silver/{current_date}/Historical_dataflow',
                                                                           schema=parquet_schema(),
                                                                           file_name_suffix=".parquet",
                                                                           codec="snappy"))
    #Read the data from the GCS and load to bigquery
    with beam.Pipeline(options=options) as p3:
        res3=(p3| "read data from the GCS">>beam.io.ReadFromParquet(file_pattern=f'gs://{bucket_name}/Silver/{current_date}/Historical_dataflow-*.parquet')
